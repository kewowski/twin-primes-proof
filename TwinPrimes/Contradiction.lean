...
theorem rho_vanishes (Hfin : twin_primes_finite) :
  ...
  let S := sieve_weight_sum D hD N
  have W_bdd : ∑ n in Finset.range (N+1), (Lambda2 n : ℝ) * selberg_weight D hD n ≤
               ∑ n in Finset.range (T+1), (Lambda2 n : ℝ) * selberg_weight D hD n := by
...
theorem rho_lower_bound :
  ...
  let c := c₁ / (2 * C)
  use c, div_pos hc₁ (mul_pos (by norm_num) hC)
  use 1
  intro N hN
  let D := N
  have hD : 1 < D := by linarith
