...
/-- Indicator function for twin primes: returns 1 if `n` and `n + 2` are both prime, else 0. -/
def Lambda2 (n : ℕ) : ℕ :=
...
/-- Twin prime weight: Λ₂(n) · wₙ, where wₙ is the Selberg sieve weight. -/
def twin_weight (D : ℕ) (hD : 1 < D) (n : ℕ) : ℝ :=
  (Lambda2 n : ℝ) * selberg_weight D hD n

/-- Weighted sum over twin primes up to N: W(N) := ∑ Λ₂(n) · wₙ. -/
